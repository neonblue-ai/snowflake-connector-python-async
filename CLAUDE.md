...
- When running benchmarks, make sure your CWD is the project root. The `.env` file is located there but the benchmark scripts should work by simply running `python benchmark/script.py ...` with no --env-file argument (picked up automatically by the script)
